// WMMA sample from ChatGPT

#include "Common.h"

namespace SampleWMMA
{
    constexpr int M = 16;
    constexpr int N = 16;
    constexpr int K = 16;
...
void WMMA()
{
    ...
    float* C_device;
    float* D_device;
    CHECK_CUDA(cudaMalloc(&A_device, A_bytes));
    CHECK_CUDA(cudaMalloc(&B_device, B_bytes));
    CHECK_CUDA(cudaMalloc(&C_device, C_bytes));
    CHECK_CUDA(cudaMalloc(&D_device, D_bytes));

    CHECK_CUDA(cudaMemcpy(A_device, A_host, A_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(B_device, B_host, B_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(C_device, C_host, C_bytes, cudaMemcpyHostToDevice));
    ...
    CHECK_CUDA(cudaMemcpy(D_host, D_device, D_bytes, cudaMemcpyDeviceToHost));
